...
subject to constraint1{q2 in Q2}: sum{h in H2} F2[h,q2] * v2[h]
                    - sum{q1 in Q1} U1[q1, q2] * r1[q1] <= 0;
subject to constraint2{h in H1}: sum{q in Q1} F1[h, q] * r1[q] == f1[h];

-----------------------------file.dat-----------------------------

set Q1 := q0 L1 R1 R1L2 R1R2 R1L3 R1;
set Q2 := q0 l1 r1;
set H1 := empty ONE TWO THREE;
set H2 := 0 one;

param F1: q0 L1 R1 R1L2 R1R2 R1L3 R1 :=
	empty 1  0  0 0 0 0 0
	ONE	 -1  1  1 0 0 0 0
	TWO   0  0 -1 0 0 1 1
	THREE 0  0 -1 0 0 1 1;
...
\textbf{Exercise 9.0.5} (Single max player, multiple max players). \textit{Provide the definition and the mathematical programming formulation of the Maxmin strategy when there is a single max player and multiple min players.}\\\\
...
\textbf{Exercise 12.0.5} (Epsilon-Nash in a normal–form game). \textit{Given a normal–form game with 2 players, find an $\varepsilon$-Nash equilibrium by means of AMPL + GUROBI.}\\
\begin{verbatim}
-----------------------------file.mod-----------------------------
...
subject to constraint2_1{i in I}: v[1] - sum{j in I} (U1[i, j] * s2[j])
                    - M[1] * (1 - b1[1]) - eps <= 0;
subject to constraint2_2{j in I}: v[2] - sum{i in I} (U1[i, j] * s1[i])
                    - M[2] * (1 - b2[j]) - eps <= 0;
                    
subject to contraint3_1{i in I}: v[1] - sum{j in I} (U1[i, j] * s2[j]) >= 0;
subject to contraint3_2{i in I}: v[2] - sum{i in I} (U2[i, j] * s1[i]) >= 0;

subject to constraint4_1{i in I}: s1[i] == 1;
subject to constraint4_2{j in I}: s2[j] == 1;
